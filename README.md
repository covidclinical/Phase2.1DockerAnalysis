...
docker run --rm --name 4ce -d -v /SOME_LOCAL_PATH:/4ceData \
                            ...
                            dbmi/4ce-analysis:version-2.1.0
```
...
The final line of the Docker command above: ``dbmi/4ce-analysis:version-2.1.0`` specifies the image name and tag that will be run.
The image name is ``dbmi/4ce-analysis`` and the tag is the string following the colon, e.g. ``version-2.1.0``. 
...
```bash
docker run --name 4ce -v /SOME_LOCAL_PATH:/4ceData --rm -it dbmi/4ce-analysis:version-2.1.0 R
```
...
```bash
docker image rm dbmi/4ce-analysis:version-2.1.0
